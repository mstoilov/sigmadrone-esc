...
Drv8323::Drv8323(SPIMaster& spi, uint32_t cs)
	: spi_(spi)
	, cs_(cs)
{
...
uint32_t Drv8323::ReadReg(uint32_t addr)
{
	...
	spi_.spi_chip_select(cs_, true);
	data = spi_.spi_write_read16(data);
	spi_.spi_chip_select(cs_, false);
	return data & 0x7FF;
}
